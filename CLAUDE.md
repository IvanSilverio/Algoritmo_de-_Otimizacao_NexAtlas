...
- **Regressão V1:** bateria de 100 casos REA (`resultados_testes_REA.json`, via `run_test_cases.py`) — deve dar **100/100** plan+plot. Não toca na V3 (nem importa `nexatlas_router.vertical`). Status `ERRO_PORTAO` (distinto de `ERRO`) é falha EXPLICADA de portão obrigatório (ver §4), não erro silencioso — a bateria também imprime um relatório de colisões portão×coerência no final.
...
- **Gabarito de equivalência Python↔TypeScript** (`tests/ts_equivalencia/`): snapshot CONGELADO (banco + CDN numa data fixa, `hora_partida_utc` fixa por caso) que prova pro time do TS que a transcrição reproduz lateral+vertical+vento+magnético dentro de tolerância — não bit-a-bit (ver `tests/ts_equivalencia/README_equivalencia.md`). `python3 tests/ts_equivalencia/verifica_gabarito.py` roda nosso Python ao vivo contra o snapshot (referência de 100% e exemplo de como comparar); `gerar_gabarito.py` regenera quando o banco mudar de forma relevante pra algum dos 25 casos. Independente da bateria de regressão acima (essa pega dado desatualizado; o gabarito pega equivalência de código).
...
- **Pista/cabeceira — input OPCIONAL e NÃO-BLOQUEANTE** (TAREFA_pista.md, 20/08/26; substitui a política antiga de "unir todas as pistas", descartada por insegura): `build_subgraph(..., pista_origem=None, pista_destino=None)` aceita a cabeceira em uso (ex.: `"13"`) por extremo. As cabeceiras já vêm do próprio documento (`parse_portoes.py` extrai `"pistas": [...]` por regra desde a implementação original); a resolução (`portoes.pontos_obrigatorios(..., pista=...)`) só aplica regras SEM `pistas` (gerais) + as regras COM `pistas` cuja lista contém a cabeceira informada. **Sem cabeceira informada (ou informada mas sem regra que bata), o aeródromo roda SEM portão forçado naquele extremo** — nunca inventa união como fallback. `portoes.aerodromo_exige_pista(icao)` diz ao orquestrador (CLI, hoje) se deve perguntar a cabeceira ao piloto; o algoritmo em si nunca pergunta nem falha por falta de pista. **7 aeródromos afetados:** SBBH, SBNT, SBJR, SBRJ, SBJD, SBMT, SDCO (TAREFA falava em 8; só 7 têm `pistas` no documento hoje — o 8º, se existir, entra quando o Vinícius mandar o dado). Efeito colateral esperado na bateria: sem pista informada, esses aeródromos deixam de forçar um ponto que antes era sempre unido — 3 casos ficam com rota mais curta (SBBH→SBFZ, SBNT→SWDI, SBRJ→SBMT) e o relatório de colisão portão×coerência passa a refletir melhor o gabarito (003 SBBH→SBCF volta a aparecer). Nenhum erro/regressão: 100/100 mantido.
