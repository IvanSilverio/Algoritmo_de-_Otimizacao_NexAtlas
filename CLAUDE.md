...
- **Descida sempre na razão do banco (`rate_dc`/`speed_dc`), governada pela LINHA DE APROXIMAÇÃO que vem do destino** (`_descida_final` em `profile.py`): a aeronave **permanece na altitude máxima** (cruzeiro ou o corredor mais alto) e só começa a descer no **"TOD de aproximação"** — o ponto a partir do qual, descendo na razão do banco, ela chega exatamente na cota do aeródromo. Não há "cross" obrigatório no 1º corredor de chegada: ela **atravessa os corredores de chegada por baixo dos tetos** (o `higher_limit` é máximo, não piso). Formalmente: a aeronave desce para atingir uma **sequência de alvos** — cada corredor de chegada no seu `higher_limit` na ENTRADA, e o destino — sempre na razão do banco; o alvo com a descida mais urgente (menor "TOD" relativo a ele) governa. Isso faz "cross" no corredor quando ele está longe (a linha ainda está acima do teto) e travessia por baixo quando está perto. Onde a geometria força (corredor alto colado no aeródromo, ou cruzeiro alto demais numa rota curta), a descida final fica mais íngreme que o padrão — é a única opção física e é aceitável.
...
- **Terminal (`nexatlas_cli.py::_print_vertical`)** mostra, por trecho, a **razão (fpm) e a velocidade (kt)** — para verificação: num trecho comprimido a velocidade cai bem abaixo da do banco.
...
- **Descida sempre na razão do banco**, começando no **TOD de aproximação** (relativo ao aeródromo). Ver §5.
