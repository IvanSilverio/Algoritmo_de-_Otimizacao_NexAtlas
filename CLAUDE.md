...
- **Waypoint além do destino é PODADO, não substituído pela direta** (revisão das 60 rotas, 02/09/26 — "Sintoma 1": o algoritmo tocava um waypoint DEPOIS do destino e voltava, ex. caso 022 SBBI→SNGA via PONTA DA FRUTA). `v1._alem_do_destino_nm` mede quanto um ponto passa do destino no eixo origem→destino (mesma ideia do `geo.progresso_nm`, mas contra a distância TOTAL do eixo, não por perna); `v1._podar_alem_destino` caminha do FIM da rota pra trás removendo esse "rabo" e religa direto ao destino o último ponto que ainda progride — **mantém os corredores do começo**, ao contrário do fator/coerência acima (que trocam a rota INTEIRA pela direta). Só poda quando AMBAS as pernas que tocam o ponto são sintéticas/`DIRETO` — corredor real NUNCA é podado (é o que protege ITAIPU/FANTE em SBMT→SBRJ e CUMBUCO/BARRA no caso 041, que também ficam "além do destino" nesse eixo, mas por corredor real publicado) — nem um **portão obrigatório documentado**, mesmo alcançado por aresta sintética (achado ao vivo: SÃO PEDRO é portão de SWEQ e caiu na poda até essa guarda existir). Caso 022 vira `SBBI→IRAÍ→PEDREIRA→MARUMBÍ→TREVO 277→SNGA` (perde só PONTA DA FRUTA, mantém REPRESA/MARUMBÍ). `meta["poda_alem_destino"]` lista o(s) ponto(s) removido(s) (nunca escondido). Primeira versão desta correção trocava a rota inteira pela direta quando detectava o waypoint além — conceitualmente errada (perdia os corredores que já estavam certos); corrigida para podar, não substituir.
